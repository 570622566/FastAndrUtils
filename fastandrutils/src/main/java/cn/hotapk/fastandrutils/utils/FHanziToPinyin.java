...
public class FHanziToPinyin {
    ...
    public static FHanziToPinyin getInstance() {
        synchronized (FHanziToPinyin.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA)) {
                    sInstance = new FHanziToPinyin(true);
                    return sInstance;
                }
            }
            if (DEBUG) {
                Log.w(TAG, "There is no Chinese collator, FHanziToPinyin is disabled");
            }
            sInstance = new FHanziToPinyin(false);
            return sInstance;
        }
    }
