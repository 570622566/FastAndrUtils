...
function getData(tableName,db) {

   $.ajax({url: "getAllDataFromTheTable?tableName="+tableName+"&dbname="+db, success: function(result){
...
function openDatabaseAndGetTableList(db) {
   ...
   $.ajax({url: "getTableList?database="+db, success: function(result){
           ...
           for(var count = 0; count < tableList.length; count++){
             ...
             $("#table-list").append("<a href='#' data-db-name='"+db+"' data-table-name='"+tableName+"' class='list-group-item' onClick='getData(\""+ tableName +","+db+ "\");'>" +tableName + "</a>");
           }
